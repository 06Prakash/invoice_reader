...
class BusinessCredit(db.Model):
    ...
    company_id = db.Column(db.Integer, db.ForeignKey('company.id'), nullable=False, unique=True)  # Add unique=True
    credit_count = db.Column(db.Numeric(precision=10, scale=2), nullable=False, default=0.00)

    company = db.relationship('Company', backref=db.backref('business_credit', uselist=False, lazy=True))
